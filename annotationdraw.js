...
var localArray = [{
    ...
    "imgSrc": "http://upload.wikimedia.org/wikipedia/commons/4/4a/Logo_2013_Google.png",
    "x": 65,
    "y": 93,
    "canvasWidth": 1000,
    "canvasHeight": 500,
...
function prepareCanvas() {
    ...
    $('#saveImg').click(function () {
        ...
        imgObj.originalWidth = parseInt($('.resize-image').css('width'), 10);
        imgObj.originalWidth = parseInt($('.resize-image').css('height'), 10);
        //imgObj.left = ;
...
function convertImgToBase64(url, callback, width, height) {
    console.log("in convertImgToBase64");
    var img = new Image();
    ...
    //$.when(function () {
        img.onload = function () {
            ...
            tcanvas.width = width;
            tctx.drawImage(this, 0, 0, width, height);
            var dataURL = tcanvas.toDataURL('image/png');
            console.log('base64: ' + dataURL);
            //$('.resize-image').attr("src", dataURL);
            ////$('.resize-image')[0].style.visibility = 'visible';
            //resizeableImage($('.resize-image'));
...
function initDraw(jsonA) {
    ...
    for (; i < jsonA.length; i++) {
        ...
        if (jsonA[i].type == "app.img") {
            ...
            //    var iSrc = convertImgToBase64(imgInJson.imgSrc, imgInJson.originalWidth, imgInJson.originalHeight);
            //    context.drawImage(buildImgTag(iSrc), imgInJson.x, imgInJson.y);
            //});
            var iSrc;
            iSrc = convertImgToBase64(imgInJson.imgSrc, function (base64Img) {
                console.log(base64Img);
                console.log("x: " + imgInJson.x + "    y: " + imgInJson.y);
                context.drawImage(buildImgTag(base64Img), imgInJson.x, imgInJson.y);

            }, imgInJson.originalWidth, imgInJson.originalHeight);
